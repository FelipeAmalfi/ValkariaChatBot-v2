...
| `DATABASE_URL` | `postgresql://postgres:postgres@localhost:5432/valkaria` | PostgreSQL connection string |
...
Add only the variables each service needs. Never add `DATABASE_URL` to the web service.
...
- `JWT_SECRET` **must be identical** across auth-service, chat-service, and world-service.
- `OPENROUTER_API_KEY` is the only paid external service. The free tier models work for development.
